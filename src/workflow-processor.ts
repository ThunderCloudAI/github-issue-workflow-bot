...
export class WorkflowProcessor {
  ...
  async processIssue(webhookPayload: GitHubWebhook): Promise<void> {
    ...
    try {
      ...
      const errorMessage =
        error instanceof WorkflowError ? error.message : `Unexpected error: ${error.message}`;
      console.error(
        `Workflow failed for issue ${context?.issueNumber || 'unknown'}:`,
        errorMessage
      );
  ...
  private async parseWebhook(webhookPayload: GitHubWebhook): Promise<WorkflowContext> {
    ...
    const hasAccess = await this.githubService.validateRepository(
      context.owner,
      context.repository
    );
    if (!hasAccess) {
    ...
    console.log(
      `Parsed webhook for issue ${context.issueNumber} in ${context.owner}/${context.repository}`
    );
    return context;
  }
  ...
  private async assignAgent(context: WorkflowContext): Promise<WorkflowContext> {
    ...
    try {
      await this.githubService.updateIssueStatus(
        context,
        'processing',
        'Tech lead analyzing requirements...'
      );

      if (!this.config.agents.tech_lead.enabled) {
        throw new WorkflowError('Tech lead agent is disabled', 'AGENT_DISABLED', false);
      }
  ...
  private async retryWithBackoff<T>(
    ...
    for (let attempt = 0; attempt <= retryConfig.maxRetries; attempt++) {
      try {
        ...
        console.warn(
          `${operationType} failed on attempt ${attempt + 1}, retrying in ${delay}ms:`,
          error.message
        );
